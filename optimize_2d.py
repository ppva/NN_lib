...
def plot_contours(fun, xrange, yrange, contours=200):
   ...
   # ax = fig.gca()
    ...
    surf=ax.plot_surface(X,Y,Z, cmap=cm.jet,cstride=1,rstride=1,alpha=0.2) #
...
def navigate_fun(navs, plot):
    ...
    for i in range(0, len(navs[0]['x']) - 1):
        for j in range(0,len(navs)):
            ...
            plot.plot(px, py,pz, 'k', alpha=1, linewidth=2.5,
...
opts.append((optimizers.Momentum(lr=step, eps=0.9), start1))
...
#opts.append((optimizers.ConjugateGradient(lr=step,ls=amg,restart=-1,beta_f="FR"), start))
